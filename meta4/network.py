...
def create_conv_sequence_until_size(initial_shape,min_initial_channels,max_channel,target_size,kernel_shape, stride, padding):
    ...
    """
    assert len(initial_shape)==3
   
    layers = []
    dummy_input=torch.rand(1,*initial_shape)
    in_channels=initial_shape[0]
    out_channels=in_channels
    initial_shape=torch.tensor(initial_shape,dtype=torch.int)
    output_shape = initial_shape
    initial_shape=initial_shape[1:]
    if in_channels<min_initial_channels:
        out_channels=min_initial_channels
        layer=nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_shape, stride=stride, padding=padding)
        layers.append(layer)
        layers.append(nn.ReLU())
        dummy_input=layer(dummy_input)       
        output_size=torch.prod(output_shape)*out_channels
        in_channels=out_channels
        
        
    
    while True:
        if out_channels*2<=max_channel:
            out_channels*=2
        layer=nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_shape, stride=stride, padding=padding)
        layers.append(layer)
        layers.append(nn.ReLU())
        dummy_input=layer(dummy_input)
        output_size=dummy_input.numel()
        print(dummy_input.shape,output_size,target_size)
        if output_size < target_size:  # Stop if adding this layer would reduce the shape below the target
            break
        in_channels=out_channels
...
#             blocks+= [UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)]
#         # gradually reduce the number of filters from ngf * 8 to ngf
#         upsamples += [UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)]
#         upsamples += [UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer)]
#         upsamples += [UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)]
#         upsamples += [UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)]  # add the outermost layer

# def define_unet_sections(num_options_each_layer,input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
#     downsample_layers=[]
#     resnet_blocks=[]
#     upsample_layers=[]
#     define_unet_generator(input_nc, output_nc, num_downs, ngf, norm_layer, use_dropout,downsample_layers,resnet_blocks,upsample_layers)
#     return [Section("downsample",downsample_layers,num_options_each_layer),Section("unet",resnet_blocks,num_options_each_layer),Section("upsample",upsample_layers,num_options_each_layer)]
