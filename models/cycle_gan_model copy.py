...
#             self.loss_names+=[name+"_D_A",name+"_G_A",name+"_cycle_A",name+"_idt_A",name+"_D_B",name+"_G_B",name+"_cycle_B",name+"_idt_B"]
#         for name in opt.names:
#             visual_names_A+=['real_A_'+name, 'fake_B_'+name, 'rec_A_'+name]
#         for name in opt.names:
#             visual_names_B+=['real_B_'+name, 'fake_A_'+name, 'rec_B_'+name]
...
#             for name in opt.names:
#                 visual_names_A.append(name+'_idt_B')
#                 visual_names_B.append(name+'_idt_A')

#         self.visual_names = visual_names_A + visual_names_B  # combine visualizations for A and B
#         # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>.
#         if self.isTrain:
#             for name in opt.names:
#                 self.model_names+=[name+'_D_A',name+'_D_B']
...
#                 self.task_names+=[name+'_G_A','name_G_B']
...
#         for name in opt.names:
#             task_G_A=meta.task.Task(shape,self.device,[])
#             task_G_B=meta.task.Task(shape,self.device,[])
#             setattr(self,name+'_task_G_A',task_G_A)
#             setattr(self,name+'_task_G_B',task_G_B)
#             # self.task_G_As.append(task_G_A)
#             # self.task_G_Bs.append(task_G_B)
#             task_G_A.extend_sections(self.generator_sections)
#             task_G_B.extend_sections(self.generator_sections)
            
#             # encoders=[opt.downsample_step_classifier_encoder,opt.blocks_step_classifier_encoder,opt.upsample_step_classifier_encoder]
#             # for i,section in enumerate( self.generator_sections):
#             #     channels=task_G_A.dummy_features.size(1)
#             #     section.set_step_classifier_encoder(meta.network.define_resnet_encoder(encoders[i],input_channels=channels))
#             #     task_G_A.append_section(section)
#             #     task_G_B.append_section(section)
...
#                                                 opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
#                 setattr(self,name+'_netD_A',netD_A)
#                 setattr(self,name+'_netD_B',netD_B)
...
#                 optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
#                 #self.optimizers.append(self.optimizer_G)
#                 setattr(self,name+'_optimizer_D',optimizer_D)
#                 setattr(self,name+'_fake_A_pool',fake_A_pool)
#                 setattr(self,name+'_fake_B_pool',fake_B_pool)
               
#                 #setattr(self,name+'_lambda_A',opt.lambda_A)
...
#         self.fake_B=self.task_G_A.forward2(self.real_A)
#         #self.fake_B_each_section=self.task_G_A.get_results()
#         self.fake_B_steps=self.task_G_A.previous_steps
        
#         self.rec_A=self.task_G_B.forward2(self.fake_B)
#         #self.rec_A_each_section=self.task_G_B.get_results()
#         self.rec_A_steps=self.task_G_B.previous_steps
        
#         self.fake_A=self.task_G_B.forward2(self.real_B)
#         #self.fake_A_each_section=self.task_G_B.get_results()
#         self.fake_A_steps=self.task_G_B.previous_steps
        
#         self.rec_B=self.task_G_A.forward2(self.fake_A)
#         #self.rec_B_each_section=self.task_G_A.get_results()
#         self.rec_B_steps=self.task_G_A.previous_steps
...
#         if lambda_idt > 0:
#             # G_A should be identity if real_B is fed: ||G_A(B) - B||
#             self.idt_A=self.task_G_A.forward(self.real_B)
...
#             self.loss_idt_A=self.criterionIdt(self.idt_A,self.real_B)*(lambda_B*lambda_idt)
#             self.idt_A_steps=self.task_G_A.previous_steps
            
            
#             self.idt_B=self.task_G_B.forward(self.real_A)
...
#             self.loss_idt_B=self.criterionIdt(self.idt_B,self.real_A)*(lambda_A*lambda_idt)
#             self.idt_B_steps=self.task_G_B.previous_steps
...
#         self.loss_G_A=self.criterionGAN(self.netD_A(self.fake_B),True)*(self.loss_D_A.detach())
...
#         self.loss_G_B=self.criterionGAN(self.netD_B(self.fake_A),True)*(self.loss_D_B.detach())
#         # Forward cycle loss || G_B(G_A(A)) - A||
...
#         self.loss_G=self.loss_G_A+self.loss_G_B+self.loss_cycle_A+self.loss_cycle_B+self.loss_idt_A+self.loss_idt_B
#         self.task_G_A.optimize_layers2(self.loss_G)
#         #self.task_G_B.optimize_layers2(self.loss_G)
        
#         self.task_G_A.optimize_steps_classifier2(self.loss_G,self.fake_B_steps)
#         self.task_G_A.optimize_steps_classifier2(self.loss_G,self.rec_B_steps)
#         self.task_G_A.optimize_steps_classifier2(self.loss_G,self.idt_A_steps)
        
#         self.task_G_B.optimize_steps_classifier2(self.loss_G,self.fake_A_steps)
#         self.task_G_B.optimize_steps_classifier2(self.loss_G,self.rec_A_steps)
#         self.task_G_B.optimize_steps_classifier2(self.loss_G,self.idt_B_steps)
