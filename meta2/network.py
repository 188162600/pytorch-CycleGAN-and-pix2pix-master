...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    return net
def define_resnet_section(num_options_each_layer,input_nc, output_nc, ngf, norm, use_dropout=False, n_blocks=6, padding_type='reflect', init_type='normal', init_gain=0.02, gpu_ids=[]):
    downsample= Section("downsample")
    blocks= Section("resnet_blocks")
    upsample= Section("upsample")
    norm=get_norm_layer(norm)
    build_resnet_generator_section(num_options_each_layer,input_nc, output_nc, ngf, norm, use_dropout, n_blocks, padding_type,downsamples= downsample,blocks= blocks,upsamples=upsample)
